...
export async function autoHealDiskSpace(
  minFreeGb: number = 4.0,
  force: boolean = false,
): Promise<boolean> {
  const beforeStats = await getDiskStats();
  console.log(
    `💾 Current Free Disk Space: ${beforeStats.availGb} GB (${beforeStats.availPercent}% free)`,
  );
  ...
  if (existsSync(maestroTestsDir)) {
    try {
      ...
      console.log(
        `  ✓ Cleaned ${entries.length} Maestro test recordings from ~/.maestro/tests`,
      );
    } catch (err) {
      console.warn(
        `  ⚠ Could not clean Maestro tests: ${err instanceof Error ? err.message : String(err)}`,
      );
    }
  }

  // 2. Clean temporary macOS simulator and XCTest caches
  const xctestCache = join(
    homedir(),
    "Library",
    "Caches",
    "com.apple.dt.XCTest",
  ...
  // 5. Clean global Xcode DerivedData if disk space is critical
  const globalDerivedData = join(
    homedir(),
    "Library",
    "Developer",
    "Xcode",
  ...
  if (
    existsSync(globalDerivedData) &&
    (force || beforeStats.availGb < minFreeGb)
  ) {
    try {
      const ddEntries = readdirSync(globalDerivedData);
      for (const entry of ddEntries) {
        if (
          entry.includes("foodshare") ||
          entry.includes("FoodShare") ||
          entry.includes("Project-")
        ) {
          const entryPath = join(globalDerivedData, entry);
          rmSync(entryPath, { recursive: true, force: true });
          console.log(`  ✓ Pruned Xcode DerivedData: ${entry}`);
        }
      }
    } catch {
      // ignore
  ...
  const cargoIncrementalDir = join(
    process.cwd(),
    "target",
    "debug",
    "incremental",
  );
  const cargoIncrementalToolsDir = join(
    process.cwd(),
    "foodshare-tools",
    "target",
    "debug",
  ...
  const freedGb = Math.max(
    0,
    Math.round((afterStats.availGb - beforeStats.availGb) * 10) / 10,
  );
  console.log(
    `✨ Disk cleanup complete: Reclaimed ~${freedGb} GB. Available: ${afterStats.availGb} GB.`,
  );
  return true;
}
