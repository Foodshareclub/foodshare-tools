...
const wasmBytes = require('fs').readFileSync(/*turbopackIgnore: true*/ wasmPath);`;
	...
	if (content.includes(oldLoader)) {
		...
		console.log(`  ✓ Injected universal Turbopack/Next.js path resolver for ${jsName}`);
	}
}
...
async function compileWasmCrate(crate: { name: string; path: string }, target: "web" | "nodejs" | "bundler" = "bundler") {
	...
	if (code === 0) {
		...
		console.log(`✅ [${crate.name}] WASM compiled successfully in ${durationSec}s -> ${outDir}`);
		return true;
	} else {
